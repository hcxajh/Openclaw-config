...
def create_task(first_frame: str, prompt: str, model: str = "MiniMax-Hailuo-2.3", duration: int = 6, resolution: str = "1080P"):
...
def main():
    parser = argparse.ArgumentParser(description="MiniMax 图生视频")
    parser.add_argument("--first-frame", "-f", required=True, help="首帧图片 URL 或本地路径")
    parser.add_argument("--prompt", "-p", required=True, help="动态描述文本")
    parser.add_argument("--model", "-m", default="MiniMax-Hailuo-2.3", help="模型名称")
    ...
    task_id = create_task(args.first_frame, args.prompt, args.model, args.duration, args.resolution)
    print(f"\n任务已提交，任务ID: {task_id}")
