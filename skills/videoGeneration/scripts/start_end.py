...
def create_task(first_frame: str, last_frame: str, prompt: str, model: str = "MiniMax-Hailuo-02", duration: int = 6, resolution: str = "1080P"):
...
def main():
    parser = argparse.ArgumentParser(description="MiniMax 首尾帧生成视频")
    parser.add_argument("--first-frame", "-f", required=True, help="首帧图片 URL")
    parser.add_argument("--last-frame", "-l", required=True, help="尾帧图片 URL")
    parser.add_argument("--prompt", "-p", required=True, help="变化描述文本")
    parser.add_argument("--model", "-m", default="MiniMax-Hailuo-02", help="模型名称")
    ...
    task_id = create_task(args.first_frame, args.last_frame, args.prompt, args.model, args.duration, args.resolution)
    print(f"\n任务已提交，任务ID: {task_id}")
