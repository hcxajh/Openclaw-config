...
def create_task(prompt: str, model: str = "MiniMax-Hailuo-2.3", duration: int = 6, resolution: str = "1080P"):
    """创建视频生成任务"""
...
def main():
    parser = argparse.ArgumentParser(description="MiniMax 文生视频")
    ...
    if not args.no_optimize:
        ...
        print(f"使用原始提示词: {args.prompt}")
    
    # 创建输出目录
    os.makedirs(args.output, exist_ok=True)
    
    # 1. 创建任务
    task_id = create_task(args.prompt, args.model, args.duration, args.resolution)
    print(f"\n任务已提交，任务ID: {task_id}")
